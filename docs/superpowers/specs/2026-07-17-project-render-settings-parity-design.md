...
将 数字孪生 当前 Three.js r183“项目配置”完整迁移到数字孪生场景平台，包括全部枚举、条件字段、输入范围、实际新场景默认值、内置地面资源、雨雪资源，以及编辑器和发布运行时一致的 Three.js 行为。

本轮不是只复制面板外观。所有配置必须进入强类型 SceneDocument、命令历史、显式保存、发布包和 RuntimeThreeEngine；切换配置后应立即改变真实 WebGL 场景。
...
资源放在 `packages/three-engine/src/settings/assets/`，由 Vite 的 `new URL(..., import.meta.url)` 同时打包到 editor-web 和 runtime-web，不依赖 数字孪生 在线服务。
...
- 每次控件 commit 仍执行 UpdateSceneSettingsCommand，因此撤销、重做和显式保存保持一致。
