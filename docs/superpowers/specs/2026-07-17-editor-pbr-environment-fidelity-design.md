...
修复同一 GLB 在 ThreeFlowX 与当前编辑器中出现显著材质差异的问题，使编辑器默认视口在不篡改 GLTF 原始 `color`、`metalness`、`roughness` 和贴图的前提下，复现源站的环境反射、明暗层次与选中反馈。
...
对 2026-07-17 的 ThreeFlowX r183 线上构建进行只读检查后确认：
