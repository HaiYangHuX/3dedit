...
test('上传模型、多实例编辑、灯光、撤销重做与刷新还原闭环', async ({
  ...
  try {
    ...
    await assetCard.dragTo(canvasHost, {
      targetPosition: { x: 500, y: 390 },
    });
    ...
    await page.getByTestId('add-geometry-box').dragTo(canvasHost, {
      targetPosition: { x: 550, y: 420 },
    });
    await expect(canvasHost).toHaveAttribute('data-scene-object-count', '3');
    await page.getByTestId('material-type').selectOption('physical');
    await page.getByTestId('material-clearcoat').fill('0.8');
    await page.getByTestId('material-clearcoat').press('Tab');
    ...
    await page.getByTestId('add-light-point').dragTo(canvasHost, {
      targetPosition: { x: 220, y: 360 },
    });
    await expect(canvasHost).toHaveAttribute('data-scene-object-count', '4');
        ...
        nodes: Record<
          string,
          {
            ...
            components: Array<
              ...
              | { kind: 'geometry'; primitive: string }
              | { kind: 'material'; materialType: string; clearcoat: number }
            >;
          }
        >;
      };
    ...
    const materialNode = nodes.find((node) =>
      node.components.some(
        (component) =>
    ...
    expect(
      materialNode?.transform.position[1],
      '拖入几何体的中心应位于网格上方',
    ).toBe(0.5);
    expect(
      pointLight?.transform.position[1],
      '拖入灯光的默认高度应至少为 0.5',
    ).toBe(0.5);
    ...
    expect(
      materialNode?.components.find(
        (component) => component.kind === 'material',
      ),
    ).toMatchObject({ materialType: 'physical', clearcoat: 0.8 });

    await page.reload();
    await expect(canvasHost).toHaveAttribute('data-engine-ready', 'true');
    await expect(canvasHost).toHaveAttribute('data-scene-object-count', '4');
    await expect(page.locator('.scene-tree-row[data-node-id]')).toHaveCount(4);
  } finally {
